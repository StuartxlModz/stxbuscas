...
?>
<!DOCTYPE html>
<!-- saved from url=(0033)https://i-find.org/consultar/Foto -->
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><title>LKZN - Foto</title>
<link rel="stylesheet" href="./cpf_files/Painel.css">
...
if(isset($_GET['consultar'])){
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => "http://rc01.gq/v9/cpfsatart?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiOTE3MyIsInVzZXIiOiJQbGV4eXMiLCJkZXNjcmlwdGlvbiI6InBhZ291IDEgbcOqcyBwb3IgNTAgcmVhaXMifSwiaWF0IjoxNjc1NzgxNDg2LCJleHAiOjE2NzgzNzM0ODZ9.dVg-8rxVJGBtsWfau0PbOT3LVT52-NmKSVbYf2L1PnU&query=" . $cpf,
...
<tbody><tr><td class="td_titulo_tabela" colspan="4">Dados Pessoais:</td></tr>
</td></tr>
<tr bgcolor="#202020"><td colspan="2"><b>Nome:</b></td><td colspan="2"><b>CPF</b></td></tr>
<tr><td colspan="2"><?= $exec->dados->NomeCompleto; ?></td><td colspan="2"><?= $cpf; ?></td></tr>
<tr bgcolor="#202020"><td colspan="2"><b>Nome da Mãe:</b></td><td colspan="2"><b>Idade:</b></td></tr>
<tr><td colspan="2"><?= $exec->dados->NomeMae; ?></td><td colspan="2"><?= $exec->dados->Idade; ?></td></tr>
<tr bgcolor="#202020"><td colspan="2"><b>Sexo:</b></td></td><td colspan="2"><b>Identidade:</b></td></tr>
<tr><td colspan="2"><?= $exec->dados->Sexo; ?></td><td colspan="2"><?= $exec->dados->Identidade; ?></td></tr>

<tr><td class="td_titulo_tabela" colspan="4">Emails:</td></tr>
<?php

foreach ($exec->dados->Emails as $json) {


?>

<tr bgcolor="#202020"><td colspan="2"><b>Emails:</b></td><td colspan="2"><b>----</b></td></tr>
<tr><td colspan="2"><?= $json->Email; ?></td><td colspan="2">----</td></tr>
<?php } ?>
<tr><td class="td_titulo_tabela" colspan="4">Telefones:</td></tr>
<?php

foreach ($exec->dados->Telefones as $json) {


?>

<tr bgcolor="#202020"><td colspan="2"><b>Telefones:</b></td><td colspan="2"><b>Operadora</b></td></tr>
<tr><td colspan="2"><?= $json->Telefone; ?></td><td colspan="2"><?= $json->Operadora; ?></td></tr>
<?php } ?>
</tr>
</tbody></table>
</td>
...
<script type="text/javascript">
    ...
    document.title = "LKZN - " + decodeURI(window.location.href.split("/").at(-1))

    document.querySelector("body > div.centralContainer > form").onkeypress = function (e) {
    ...
    function pageUnloaded() {
        ...
        document.getElementById("realizar").style.backgroundImage = ""
        document.getElementById("realizar").style.backgroundSize = ""
        document.getElementById("realizar").innerHTML = "Consultar"
    };

    window.addEventListener("unload", pageUnloaded, false);
    window.addEventListener("pagehide", pageUnloaded, false);

    function RealizarConsulta() {

        if (document.getElementById("realizar").innerHTML != "") {

            //programação antiga mas pode ser mantida pra checar se o user não inputou nada
            inputs = document.getElementsByClassName("valueConsulta")
            p = ""
            for (i = 0; i < inputs.length; i++) {
                if (inputs[i].getAttribute("type") == "checkbox") {
                    p = p + inputs[i].checked + ","
                } else {
                    p = p + inputs[i].value.split(",").join("") + ","
                }
            }
            p = p.slice(0, -1)
            p = p.split("/").join("")
            ...
            try {
                document.getElementById("realizar").style.backgroundImage = "url(https://i-find.org/static/svgs/Loading.svg)"
                document.getElementById("realizar").style.backgroundSize = "100% 100%"
                document.getElementById("realizar").innerHTML = ""
            } catch { } //parece estar dando problema em alguns navegadores antigos

            document.querySelector("body > div.centralContainer > form").submit()
    ...
    firstText = false
    for (i = 0; i < document.getElementsByClassName("valueConsulta").length; i++) {

        if (document.getElementsByClassName("valueConsulta")[i].getAttribute("type") == "text" && firstText == false) {
            document.getElementsByClassName("valueConsulta")[i].focus()
            firstText = true
        }


        document.getElementsByClassName("valueConsulta")[i].addEventListener("keyup", function (event) {
            if (event.keyCode === 13) {
                RealizarConsulta()
            }
        })
    ...
    function GoToConsulta() {
        ...
        window.location.href = "/consultar/" + this.innerText
    }


    function toggleView() {

        //if (document.querySelector("body > div.fatherCenter > div > div").style.display == "none") {
        //    document.querySelector("body > div.fatherCenter > div > div").style.display = 'flex'
        //    document.querySelector("body > div.fatherCenter > div > button").innerHTML = "▲"
        //} else {
        //    document.querySelector("body > div.fatherCenter > div > div").style.display = 'none'
        //    document.querySelector("body > div.fatherCenter > div > button").innerHTML = "▼"
        //}

        if (document.querySelector("body > div.fatherCenter > div > div").style.height == "0px") {
            nmbCh = document.querySelector("body > div.fatherCenter > div > div").children.length
            document.querySelector("body > div.fatherCenter > div > div").style.height = (nmbCh * 3.25) + "vh"
            document.querySelector("body > div.fatherCenter > div > button").innerHTML = "▲"
        } else {
            document.querySelector("body > div.fatherCenter > div > div").style.height = "0px"
            document.querySelector("body > div.fatherCenter > div > button").innerHTML = "▼"

        }
    }

    quickButtonsConsulta = document.getElementsByClassName("btnConsulMenuList")
    for (i = 0; i < quickButtonsConsulta.length; i++) {
        quickButtonsConsulta[i].addEventListener('click', GoToConsulta, false);
    }
